...
class TCVAE():
    def __init__(self, hparams, mode):
        ...
        self.max_story_length = 215
        self.max_length = 110
        self.second_max_length = 220
        ...
        if self.mode != tf.contrib.learn.ModeKeys.INFER:
            ...
            self.second_input_ids = tf.placeholder(tf.int32, [None, None])
            self.input_scopes = tf.placeholder(tf.int32, [None, None])
            self.second_input_scopes = tf.placeholder(tf.int32, [None, None])
            self.input_positions = tf.placeholder(tf.int32, [None, None])
            self.second_input_positions = tf.placeholder(tf.int32, [None, None])
            self.input_masks = tf.placeholder(tf.int32, [None, None, None])
            self.input_lens = tf.placeholder(tf.int32, [None])
            self.second_input_lens = tf.placeholder(tf.int32, [None])
        ...
        with tf.variable_scope("encoder") as scope:
            ...
            self.second_word_emb = tf.nn.embedding_lookup(self.word_embeddings, self.second_input_ids)
            self.second_scope_emb = tf.nn.embedding_lookup(self.scope_embeddings, self.second_input_scopes)
            self.second_pos_emb = positional_encoding(self.second_input_positions, self.batch_size, self.second_max_length, int(self.emb_dim/2))

            # self.embs = self.word_emb + self.scope_emb + self.pos_emb
            self.embs = tf.concat([self.word_emb, self.scope_emb, self.pos_emb], axis=2)
            self.second_embs = tf.concat([self.second_word_emb,self.second_scope_emb, self.second_pos_emb], axis=2)
            inputs = self.input_layer(self.embs)
            second_inputs = self.input_layer(self.second_embs)
            ...
            for i in range(self.encoder_num_layers):
                with tf.variable_scope("encoder_num_layers_{}".format(i)):
                # 2-encoder
                    second_outputs = multihead_attention(queries=second_inputs,
                                                              keys=second_inputs,
                                                              query_length=self.second_input_lens,
                                                              key_length=self.second_input_lens,
                    ...
                    second_outputs = second_outputs + second_inputs
                    second_inputs = normalize(second_outputs)

                    second_outputs = feedforward(second_inputs, [self.num_units * 2, self.num_units], is_training=self.is_training,
            ...
            for i in range(self.decoder_num_layers):
                with tf.variable_scope("decoder_num_layers_{}".format(i)):
    ...
    #处理输入的数据
    def get_batch(self, first_data, second_data, no_random=False, id=0, which=0):
        ...
        for i in range(hparams.batch_size):
            if no_random:
                x = first_data[(id + i) % len(first_data)]
                y = second_data[(id + i) % len(second_data)]
                #which_stn = (id + i) % 5
                which_stn = 1
                # which_stn = which
            else:
                temp = random.randint(0, len(first_data) - 1)
                x = first_data[temp]
                y = second_data[temp]
                which_stn = 1
            ...
            #second_input
            second_input_id.append(GO_ID)
            second_input_scope.append(0)
            second_input_position.append(0)
            for k in range(0, len(y[0])):
                second_input_id.append(y[0][k])
                second_input_scope.append(0)
                second_input_position.append(k + 1)
            second_input_id.append(EOS_ID)
            second_input_scope.append(0)
            second_input_position.append(len(y[0]) + 1)
            second_input_lens.append(len(second_input_id))
            for k in range(0, self.second_max_length - second_input_lens[i]):
                second_input_id.append(PAD_ID)
                second_input_scope.append(0)
                second_input_position.append(0)
            ...
            second_input_ids.append(second_input_id)
            second_input_scopes.append(second_input_scope)
            second_input_positions.append(second_input_position)
        ...
        return input_ids, input_scopes, input_positions, input_masks, input_lens, input_which, targets, weights, input_windows, second_input_ids, second_input_scopes, second_input_positions, second_input_lens

    def train_step(self, sess, first_data, second_data):
        input_ids, input_scopes, input_positions, input_masks, input_lens, input_which, targets, weights, input_windows, second_input_ids, second_input_scopes, second_input_positions, second_input_lens = self.get_batch(first_data, second_data)
        feed = {
            self.input_ids:input_ids,
            self.second_input_ids:second_input_ids,
            self.input_scopes:input_scopes,
            self.second_input_scopes: second_input_scopes,
            self.input_positions:input_positions,
            self.second_input_positions:second_input_positions,
            self.input_masks:input_masks,
            self.input_lens:input_lens,
            self.second_input_lens:second_input_lens,
    ...
    def eval_step(self, sess, first_data, second_data, no_random=False, id=0):
        input_ids, input_scopes, input_positions, input_masks, input_lens, input_which, targets, weights, input_windows, second_input_ids, second_input_scopes, second_input_positions, second_input_lens = self.get_batch(
            first_data, second_data, no_random, id)
        feed = {
            self.input_ids: input_ids,
            self.second_input_ids: second_input_ids,
            self.input_scopes: input_scopes,
    ...
    def infer_step(self, sess, first_data, second_data, no_random=False, id=0, which=0):
        tf.enable_eager_execution()
        input_ids, input_scopes, input_positions, input_masks, input_lens, input_which, targets, weights, input_windows, second_input_ids, second_input_scopes, second_input_positions, second_input_lens = self.get_batch(
            first_data, second_data, no_random, id, which=which)

        input_ids = np.asarray(input_ids).repeat(self.hparams.beam_size,axis=0).tolist()
        second_input_ids = np.asarray(second_input_ids).repeat(self.hparams.beam_size,axis=0).tolist()
        input_scopes = np.asarray(input_scopes).repeat(self.hparams.beam_size,axis=0).tolist()
        second_input_scopes = np.asarray(second_input_scopes).repeat(self.hparams.beam_size,axis=0).tolist()
        input_positions = np.asarray(input_positions).repeat(self.hparams.beam_size,axis=0).tolist()
        second_input_positions = np.asarray(second_input_positions).repeat(self.hparams.beam_size,axis=0).tolist()
        ...
        second_input_lens = np.asarray(second_input_lens).repeat(self.hparams.beam_size,axis=0).tolist()
