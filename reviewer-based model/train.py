...
def add_arguments(parser):
    ...
    parser.add_argument("--second_train_data", type=str, default="second_training",
                        help="Training data path")

    parser.add_argument("--second_valid_data", type=str, default="second_dev",
                        help="Valid data path")

    parser.add_argument("--second_test_data", type=str, default="second_test",
                        help="Test data path")
    ...
    parser.add_argument("--from_vocab_size", type=int, default=9565, help="source vocabulary size")
    parser.add_argument("--to_vocab_size", type=int, default=9565, help="target vocabulary size")
    parser.add_argument("--encoder_num_layers", type=int, default=2, help="Number of layers in the model for encoder")
    parser.add_argument("--decoder_num_layers", type=int, default=4, help="Number of layers in the model for decoder")
    ...
    parser.add_argument("--emb_dim", type=int, default=2048, help="Dimension of word embedding")
    ...
    parser.add_argument("--beam_size", type=int, default=1, help="Beam size to use during infer")
...
def create_hparams(flags):
    return tf.contrib.training.HParams(
        ...
        second_train_data=flags.second_train_data,
        second_valid_data=flags.second_valid_data,
        second_test_data=flags.second_test_data,
...
def train(hparams):
    ...
    train_data = read_data("data/train1.ids")
    valid_data = read_data("data/valid1.ids")
    test_data = read_data("data/test1.ids")
    second_train_data = read_data("data/train2.ids")
    second_valid_data = read_data("data/valid2.ids")
    second_test_data = read_data("data/test2.ids")
    ...
    while global_step <= 380000:
        ...
        step_loss, global_step, predict_count = train_model.model.train_step(train_sess, train_data, second_train_data)
        ...
        if  global_step % 3000 == 0:
            ...
            for id in range(0, int(len(test_data)/hparams.batch_size)):
                step_loss, predict_count = eval_model.model.eval_step(eval_sess, test_data, second_test_data, no_random=True, id=id * hparams.batch_size)
            ...
            f3 = open("output/" + x + "/bleu2_file" + str(global_step),"w", encoding="utf-8")
            for id in range(0, int(len(valid_data) / hparams.batch_size)):

                given, answer, predict, predict_nobeam = infer_model.model.infer_step(infer_sess, test_data, second_test_data, no_random=True,
                                                                      id=id * hparams.batch_size)

                for i in range(hparams.batch_size * hparams.beam_size):
                    ...
                    if i % hparams.beam_size == 0:
                        sample_output = predict_nobeam[i]
                        if hparams.EOS_ID in sample_output:
                            sample_output = sample_output[:sample_output.index(hparams.EOS_ID)]
                        pred_nobeam = []
                        for output in sample_output:
                            pred_nobeam.append(tf.compat.as_str(rev_to_vocab[output]))
                        if id == 0 and i < 4:
                            print("pred_nobeam: ", " ".join(pred_nobeam))
                        f3.write(" ".join(pred_nobeam) + "\n")
            ...
            f2.close()
            f3.close()
            hyp_file = "output/" + x + "/bleu2_file" + str(global_step)
            ref_file = "output/" + x + "/ref2_file" + str(global_step)
            result = os.popen("python tester.py --ref " + ref_file + " --hyp " + hyp_file + " --global_step " + str(global_step))

            f4 = open("output/" + x + "/bleu2_file" + str(global_step), "r", encoding="utf-8")
            dic1 = {}
            dic2 = {}
